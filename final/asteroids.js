...
// A simple inheritance mechanism to emulate classes
function extend(ChildClass, ParentClass) {
  var parent = new ParentClass();                   // get a ParentClass instance
  ChildClass.prototype = parent;                    // set the ChildClass prototype to the parent instance
  ChildClass.prototype.super = parent.constructor;  // when super is called it will be the parent constructor
  ChildClass.prototype.constructor = ChildClass;    // the child constructor is retained
}
...
Ship.prototype.draw = function(c, guide) {
  ...
  draw_ship(c, 0.25 * Math.PI, this.radius, -0.25, Math.PI * 0.1, this.thruster_on, guide);
  c.restore();
}
...
Ship.prototype.update = function(elapsed, c) {
  ...
  // call the Mass update function
  Mass.prototype.update.apply(this, arguments);
}

Ship.prototype.projectile = function(elapsed) {
  var p = new Projectile(this.weapon_mass, this.projectile_life,
...
function Asteroid(mass, x, y, x_speed, y_speed, rotation_speed) {
  ...
  this.segments = Math.ceil(this.circumference / 5);
...
Asteroid.prototype.child = function(mass) {
  return new Asteroid(mass, this.x, this.y, this.x_speed, this.y_speed, this.rotation_speed)
}

function Projectile(mass, lifetime, x, y, x_speed, y_speed, rotation_speed) {
  var density = 0.02; // low density means we can see very light projectiles
  var radius = Math.sqrt((mass / density) / Math.PI);
  this.super(mass, radius, x, y, 0, x_speed, y_speed, rotation_speed);
...
function NumberIndicator(label, x, y, fix, pt) {
  this.label = label + ": ";
  this.x = x;
  this.y = y;
  this.fix = fix;
...
NumberIndicator.prototype.draw = function(c, value) {
  ...
  var offset = c.measureText(this.label).width;
  c.fillText(this.label + value.toFixed(this.fix), this.x, this.y + this.pt - 1);
  c.restore();
}
